...
track_drone_router = APIRouter()
track_drone_om2m_request_sender = Om2mRequestSender()
...
@track_drone_router.get("/{index}/info", status_code=200)
async def get_drone_info(index: int):
    # get latest data from mn resource tree
    response = track_drone_om2m_request_sender.get_latest_content_instance(
        f"{config.IN_URL}/~/in-cse/in-name",
        f"track_drone_{index+1}",
        "info",
    )
